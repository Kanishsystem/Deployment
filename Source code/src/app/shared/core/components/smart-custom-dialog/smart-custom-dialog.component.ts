...
export class SmartCustomDialogComponent {
  ...
  get urliframe(){
    console.log("iframe url  payload " , this.data.iframe);
  ...
  getPdfData(){
    ...
    */
    let url = this.api.getFullUrl(this.data.iframe_payload);
    this.iframeData = url;
   // document.getElementById('base64Iframe').setAttribute('src', url);
   
   this.api.smartPost(this.data.iframe,payload).subscribe((res:any)=>{
      this.pdfData = res.content;
      //this.iframeData= 'data:application/octet-stream;base64,' +  this.pdfData;
      //document.getElementById('base64Iframe').setAttribute('src', this.iframeData);
      //console.log("pdf data " , this.pdfData);
   })
    /*
