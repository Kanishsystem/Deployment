...
@Component({
  selector: 'app-custom-complaints',
  templateUrl: './custom-complaints.component.html',
  styleUrls: ['./custom-complaints.component.css']
})
export class CustomComplaintsComponent {
  ...
  mode_data = {
    emp: {
      ...
      url: 'SITE_COMPLAINT_GET_ALL_USER',
    },  
    admin: {
      title: 'Electrical Complaints-Admin',
      url: 'SITE_COMPLAINT_GET_ALL',
    },
  };
  /**
   * get mod data
  ...
  get siteTitle() {
    return this.singleComplaintData?.complaint_type + "-User"
  }
  ...
  getTableData() {
    ...
    this.api.smartPost(tableUrl,{type:this.catType}).subscribe((res: any) => {
      this.tableData = res;
    });
  }

  getSingleComplaintTypeData(id:number) {   
    let get_one = get_api_route("SITE_COMPLAINT_TYPE_GET_ONE");
    this.api.smartPost(get_one, { id: id }).subscribe((res: any) => {     
      this.singleComplaintData = res; 
      console.log(" complaint type " , this.singleComplaintData)   
    });
  }
  ...
  ngOnInit(): void {
    this.routeSubscription =  this.route.params.subscribe(params => {
      ...
      this.getTableData();
      this.getSingleComplaintTypeData(this.catType);
    });
  ...
  openViewForm(data) {
    ...
    let get_one = get_api_route('SITE_COMPLAINT_ONE');
    this.api.smartPost(get_one, { id: id }).subscribe((res: any) => {
  ...
  openForm(data: any = null) {
    this.formData = {};
    let options = {
      title:  "New Complaint Form - " + this.singleComplaintData.complaint_type ,
  ...
  editForm(data: any = null) {
    let options = {
      title: 'Complaint Update Form -' +  this.singleComplaintData.complaint_type,
  ...
  delete_one() {
    let deleteUrl = get_api_route('SITE_COMPLAINT_DELETE_ONE');
  ...
  app_status_update_one(status: string) {
    ...
    let deleteUrl = get_api_route('SITE_COMPLAINT_UPDATE');
    this.api
  ...
  submitData(data) {
    ...
    data["type"] = this.catType;
    this.api.smartPost('SITE_COMPLAINT_INSERT', data).subscribe((res: any) => {
  ...
  updateDate(data) {
    let id = data.ID !== undefined ? data.ID : 0;
    let update_url = get_api_route('SITE_COMPLAINT_UPDATE');
    data['id'] = id;
