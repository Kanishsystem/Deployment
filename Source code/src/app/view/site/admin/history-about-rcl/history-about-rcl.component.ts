...
@Component({
  selector: 'app-history-about-rcl',
  templateUrl: './history-about-rcl.component.html',
  styleUrls: ['./history-about-rcl.component.css']
})
export class HistoryAboutRclComponent {
  ...
  ngOnInit(): void {
    this.imageUrl = get_api_full_route("SITE_HISTORY_GET_IMAGE")
    this.createTable();
    this.getTableData();
  ...
  openEditForm(data) {
    ...
    let get_one = get_api_route("SITE_HISTORY_GET_ONE");
  ...
  getTableData() {
    this.api.smartGet('SITE_HISTORY_GET_ALL').subscribe((res: any) => {
      this.tableData = res;
    });
  }
  ...
  // submitData(data) {
  //   this.api.smartPost('SITE_HISTORY_INSERT', data).subscribe((res: any) => {
  ...
  submitData(data) {
    ...
    this.api.smartPost('SITE_HISTORY_INSERT',  formData,
    false,
    false,
  ...
  delete_one() {
    let deleteUrl = get_api_route("SITE_HISTORY_DELETE_ONE");
