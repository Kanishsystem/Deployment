...
import { SmartTableConfig, SmartTableMainConfig, SmartTableColumnConfig } from 'src/app/shared/core/SmartInterfaces/SmartTableNewInterface';
import { SmartDialogService } from 'src/app/shared/core/services/smart-dialog.service';

@Component({
  selector: 'app-shoutdown',
  templateUrl: './shoutdown.component.html',
  styleUrls: ['./shoutdown.component.css']
})
export class ShoutdownComponent {
  ...
  mode_data = {
    emp: {
      title: 'ACV Shutdown Requisition',
      url: 'SITE_ACV_SHUTDOWN_GET_ALL_USER',
    },
    app: {
      title: 'Electrical Complaints-Approval',
      url: 'SITE_ACV_SHUTDOWN_GET_ALL_APP',
    },
    admin: {
      title: 'ACV Shutdown Requisition-Admin',
      url: 'SITE_ACV_SHUTDOWN_GET_ALL',
    },
  };
  /**
   * get mod data
  ...
  openViewForm(data) {
    ...
    let get_one = get_api_route('SITE_ACV_SHUTDOWN_ONE');
  ...
  openForm(data: any = null) {
    let options = {
      title: 'ACV Shutdown Requisition Form',
  ...
  editForm(data: any = null) {
    let options = {
      title: 'ACV Shutdown Requisition Update Form',
  ...
  delete_one() {
    let deleteUrl = get_api_route('SITE_ACV_SHUTDOWN_DELETE_ONE');
  ...
  app_status_update_one(status: string) {
    ...
    let deleteUrl = get_api_route('SITE_ACV_SHUTDOWN_UPDATE_APP');
    this.api
  ...
  submitData(data) {
    ...
    this.api.smartPost('SITE_ACV_SHUTDOWN_INSERT', formData, false, false, "multipart/form-data").subscribe((res: any) => {
  ...
  updateDate(data) {
    let id = data.ID !== undefined ? data.ID : 0;
    let update_url = get_api_route('SITE_ACV_SHUTDOWN_UPDATE');
    data['id'] = id;
  ...
  openPdfView(data) {
    ...
    let options = {
      title:"PDF Viewer",
      iframe:get_api_route("SITE_ACV_SHUTDOWN_GET_PDF")
    };
