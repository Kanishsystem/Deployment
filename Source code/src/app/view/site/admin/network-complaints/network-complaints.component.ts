...
@Component({
  selector: 'app-network-complaints',
  templateUrl: './network-complaints.component.html',
  styleUrls: ['./network-complaints.component.css']
})
export class NetworkComplaintsComponent {
  ...
  mode_data = {
    emp:{
      title:" My Network Complaints",
      url:"SITE_NETWORK_GET_ALL_USER"
    },
    
    app:{
      title:"Network Complaints-Approval",
      url:"SITE_NETWORK_GET_ALL_APP"
    },
    admin:{
      title:"Network Complaints-Admin",
      url:"SITE_NETWORK_GET_ALL"
    },
  }
  /**
   * get mod data
  ...
  openViewForm(data) {
    ...
    let get_one = get_api_route('SITE_NETWORK_ONE');
  ...
  editForm(data: any = null) {
    let options = {
      title: 'Network Complaint Update Form',
  ...
  delete_one() {
    let deleteUrl = get_api_route('SITE_NETWORK_DELETE_ONE');
  ...
  submitData(data) {
    this.api.smartPost('SITE_NETWORK_INSERT', data).subscribe((res: any) => {
  ...
  updateDate(data) {
    let id = data.ID !== undefined ? data.ID : 0;
    let update_url = get_api_route('SITE_NETWORK_UPDATE');
    data['id'] = id;
