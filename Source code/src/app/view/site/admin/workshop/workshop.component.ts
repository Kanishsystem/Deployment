...
export class WorkshopComponent {
  ...
  mode_data = {
    emp: {
      title: 'Workshop Requisition',
      url: 'SITE_WORKSHOP_GET_ALL_USER',
    },
    app: {
      title: 'Electrical Complaints-Approval',
      url: 'SITE_WORKSHOP_GET_ALL_APP',
    },
    admin: {
      title: 'Workshop Requisition-Admin',
      url: 'SITE_WORKSHOP_GET_ALL',
    },
  };
  /**
   * get mod data
    ...
    let form_fileds: SmartFormField[] = [
      ...
      },
      {
        ...
        validations: [
          {
            type: 'required',
            msg: ' Free Issue Material is Required',
          },
        ],  
      },
      {
        ...
        hideFunction: (item, get_value) => {
          ...
          return get_value('free_material') == 1 ? false : true;
        },
        validations: [
          {
            type: 'required',
            msg: ' Description Material is Required',
          },
          // {
          //   type:'maxLength',
          //   msg:"",
  ...
  openForm(data: any = null) {
    let options = {
      title: 'Workshop Requisition Form',
  ...
  editForm(data: any = null) {
    let options = {
      title: 'Workshop Requisition Update Form',
  ...
  delete_one() {
    let deleteUrl = get_api_route('SITE_WORKSHOP_DELETE_ONE');
  ...
  app_status_update_one(status: string) {
    ...
    let deleteUrl = get_api_route('SITE_WORKSHOP_UPDATE_APP');
    this.api
  ...
  submitData(data) {
    ...
    this.api.smartPost('SITE_WORKSHOP_INSERT', formData, false, false, "multipart/form-data").subscribe((res: any) => {
      this.smartDialog.closeDialog();
      this.notify.success("Submitted successfully");
      this.getTableData();
    });
    /*
    this.api.smartPost('SITE_WORKSHOP_INSERT', data).subscribe((res: any) => {
      this.smartDialog.closeDialog();
      this.notify.success('Submitted successfully');
      this.getTableData();
    });
  ...
  updateDate(data) {
    let id = data.ID !== undefined ? data.ID : 0;
    let update_url = get_api_route('SITE_WORKSHOP_UPDATE');
    data['id'] = id;
  ...
  openPdfView(data) {
    ...
    let options = {
      title:"PDF Viewer",
      iframe:get_api_route("SITE_WORKSHOP_GET_PDF")
    };
