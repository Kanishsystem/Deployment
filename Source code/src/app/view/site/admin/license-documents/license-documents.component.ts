...
@Component({
  selector: 'app-license-documents',
  templateUrl: './license-documents.component.html',
  styleUrls: ['./license-documents.component.css']
})
export class LicenseDocumentsComponent {
  ...
  openEditForm(data) {
    ...
    let get_one = get_api_route("SITE_LICENSE_DOCUMENT_GET_ONE");
  ...
  getTableData() {
    this.api.smartGet('SITE_LICENSE_DOCUMENT_GET_ALL').subscribe((res: any) => {
      this.tableData = res;
    });
  }
  ...
  submitData(data) {
    ...
    this.api.smartPost('SITE_LICENSE_DOCUMENT_INSERT', formData, false, false, "multipart/form-data").subscribe((res: any) => {
      this.smartDialog.closeDialog();
      this.notify.success("Submitted successfully");
      this.getTableData();
    });
  ...
  updateDate(data) {
    let id = data.ID !== undefined ? data.ID : 0;
    let update_url = get_api_route("SITE_LICENSE_DOCUMENT_UPDATE");
    data["id"] = id;
  ...
  delete_one() {
    let deleteUrl = get_api_route("SITE_LICENSE_DOCUMENT_DELETE_ONE");
  ...
  openPdfView(data) {
    ...
    let options = {
      ...
      iframe:get_api_route("SITE_LICENSE_DOCUMENT_GET_DOCUMENT")
    };
  ...
  downLoadFile(id,fileName:string){
    let payload = {id:id};
    this.api.smartPost("SITE_LICENSE_DOCUMENT_GET_DOCUMENT",payload).subscribe((res:any)=>{
